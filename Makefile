...
#    make uishot     - render the editor to build/KeyGlo-ui.png headlessly
...
# edits.
PLUGIN_SRCS := $(wildcard $(SRC)/*.cpp) $(wildcard $(SRC)/dsp/*.cpp) $(wildcard $(SRC)/analysis/*.cpp) \
...
$(SA_BUNDLE): $(CONFIG_STAMP) $(SA_OBJS) $(SA_OBJ) $(ROOT)/packaging/Standalone-Info.plist $(ASSET_FILES)
	...
	@test -f $(ROOT)/packaging/KeyGlo.icns \
	  && cp $(ROOT)/packaging/KeyGlo.icns $@/Contents/Resources/KeyGlo.icns \
	  || echo "  WARNING          no KeyGlo.icns - run 'make icon' (app will show a generic icon)"
	$(call COPY_ASSETS,$@)
...
icon: $(TOOLS)/makeicon
	@$(TOOLS)/makeicon $(LAYOUT) $(BUILD)/KeyGlo.iconset
	@iconutil -c icns $(BUILD)/KeyGlo.iconset -o $(ROOT)/packaging/KeyGlo.icns
	@echo "  ICNS             packaging/KeyGlo.icns ($$(du -h $(ROOT)/packaging/KeyGlo.icns | cut -f1), layout=$(LAYOUT))"
...
# -----------------------------------------------------------------------------
PKG_DIR      := $(BUILD)/pkg
INSTALLER    := $(DIST)/KeyGlo-$(VERSION).pkg
...
installer:
	...
	@# across, and pkgbuild then emits AppleDouble "._KeyGlo.vst3" siblings
	...
	@ditto --norsrc --noextattr $(VST3_BUNDLE) "$(PKG_DIR)/root-vst3/Library/Audio/Plug-Ins/VST3/KeyGlo.vst3"
	@ditto --norsrc --noextattr $(AU_BUNDLE)   "$(PKG_DIR)/root-au/Library/Audio/Plug-Ins/Components/KeyGlo.component"
	@ditto --norsrc --noextattr $(SA_BUNDLE)   "$(PKG_DIR)/root-app/Applications/KeyGlo.app"
	...
	@codesign --verify --deep --strict "$(PKG_DIR)/root-vst3/Library/Audio/Plug-Ins/VST3/KeyGlo.vst3" \
	  || { echo "  staged VST3 signature is broken"; exit 1; }
	@codesign --verify --deep --strict "$(PKG_DIR)/root-app/Applications/KeyGlo.app" \
	  || { echo "  staged app signature is broken"; exit 1; }
	@pkgbuild --root $(PKG_DIR)/root-vst3 --identifier com.diamondloopz.keyglo.vst3 \
	   --version $(VERSION) --install-location / $(PKG_DIR)/KeyGlo-VST3.pkg >/dev/null
	@pkgbuild --root $(PKG_DIR)/root-au --identifier com.diamondloopz.keyglo.au \
	   --version $(VERSION) --install-location / $(PKG_DIR)/KeyGlo-AU.pkg >/dev/null
	@pkgbuild --root $(PKG_DIR)/root-app --identifier com.diamondloopz.keyglo.app \
	   --version $(VERSION) --install-location / $(PKG_DIR)/KeyGlo-App.pkg >/dev/null
	...
	@echo "    xcrun notarytool store-credentials KeyGlo --apple-id YOU@EXAMPLE.COM \\"
...
release:
	...
	@xcrun stapler validate "$(INSTALLER)" >/dev/null 2>&1 || { \
	  ...
	  echo "    xcrun notarytool store-credentials KeyGlo --apple-id YOU \\"; \
	  echo "      --team-id 922D43C6FJ --password APP-SPECIFIC-PASSWORD"; \
	  echo "    make notarize && make release"; \
	  exit 1; }
	@echo "  installer is notarised and stapled"
	@rm -rf "$(BUILD)/releasezip" && mkdir -p "$(BUILD)/releasezip/KeyGlo $(VERSION)"
	@cp "$(INSTALLER)" "$(BUILD)/releasezip/KeyGlo $(VERSION)/Install KeyGlo.pkg"
	@cp $(ROOT)/packaging/release/ReadMeFirst.txt \
	    "$(BUILD)/releasezip/KeyGlo $(VERSION)/Read Me First.txt"
	@cp $(ROOT)/packaging/release/License.txt \
	    "$(BUILD)/releasezip/KeyGlo $(VERSION)/License.txt"
	@rm -f "$(BUILD)/KeyGlo-$(VERSION)-macOS.zip"
	@cd "$(BUILD)/releasezip" && zip -rXq "$(BUILD)/KeyGlo-$(VERSION)-macOS.zip" \
	    "KeyGlo $(VERSION)"
	@echo "  $(BUILD)/KeyGlo-$(VERSION)-macOS.zip ($$(du -h "$(BUILD)/KeyGlo-$(VERSION)-macOS.zip" | cut -f1))"

notarize: $(INSTALLER)
	@echo "Submitting to Apple. This usually takes a few minutes."
	xcrun notarytool submit $(INSTALLER) --keychain-profile KeyGlo --wait
